...
<footer>Only the top 20 rows are shown. At most 256 page labels and 128 referral labels are retained per UTC day, with excess or rejected values grouped as (other). Date-specific reading URLs share their topic page. Retention is 90 days. The collector normally flushes every 10 seconds. After write failures, older counters may remain buffered or be lost; an unexpected process exit can lose buffered activity. Graceful shutdown attempts a bounded flush, which can fail. Re-run <code>just traffic 7</code>, <code>just traffic 30</code>, or <code>just traffic 90</code> with REMOTE set to change the period.</footer>
</main></body></html>`))
